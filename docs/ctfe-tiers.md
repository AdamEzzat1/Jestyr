...
| 2 | `comptime { … }` blocks in user syntax | **done in the Rust reference** — G2; the port **parses** it (M1) and its interpreter **folds scalars** (M2, `examples/std/ctfe.jtr`); typeck-fold + cgen mirrors outstanding |
