...
| P5 cgen | `src/cgen.rs` (~10.8K) | 🟡 **UNDERWAY (44 files)** — latest: struct methods (15), guarded match + generic-enum instances (16, the first monomorphization), owned String/Cow/Builder intrinsics (17), alloc/utf8 boundary + str byte access (18), f-strings via token re-scan (19), binary literals + template-fn suppression (20), scalar match + or-patterns (21); details in the P5 handoff progress log. Earlier: — +incr 7 (Assign/break/continue/`for` range+while+infinite), +incr 8 (**slice/pointer machinery**: `JestyrSlice_<T>` typedefs + bounds-checked indexing + refinement elision + slice `for`, Ptr/Deref, `unsafe{}`/block stmts, `invariant`, alloc/slice/free intrinsics, `mut`/`out` args by `&()`; `loops`+`slices`), +incr 9 (**arrays + consts**: `JestyrArr_<m>_<N>` typedefs scanning the Checker `c.et` first — the `[5]i32` ghost typedef — then fn sigs; ArrayLit stmt-expr, array indexing, `.len`→`((size_t)N)`, array `for`; real consts section w/ brace-init for array literals; `array_lit`), +incr 10 (**error sets**: Result type `T!`→`JestyrResult_<mangle(ok)>`, per-ok-type result typedefs, fallible fn sigs, whole-program error-tag map, `ok`/`err`/`unwrap`/`is_err` + `?` Try stmt-expr; `errors`). +incr 11 (**enums + flat match**: tag-enum + payload-union defs, nullary/payload variant construction, enum->int `.tag` casts, `switch (jm_{n}.tag)` with payload projection + default arms + `__builtin_unreachable()`; guards/nesting/niche/scalar-scrutinee deferred). +incrs 12-14 (near-miss sweeps: param refinements, struct-lit spread+defaults, @packed/@align, extern "c" protos+bare calls, @volatile/@address MMIO, bytes/try_from_utf8, realloc_i32, return-unsafe, bit-fields, _Alignof/offsetof, requires/ensures contracts). **NEXT:** records/methods (mcalls) -> guarded match + generic enums -> FString (needs parser interp sub-exprs) -> scalar/nested match → enums/match (the large one) → traits/dyn → generics/mono → drop glue → closures → concurrency. Earlier — `examples/std/cgen.jtr` + golden `jestyr_cgen_matches_reference`; **byte-identical** on `{hello, bench_fib, eq_fold, distinct, compute, copy_optin, io, str_ops, substr, union}` (prelude+section skeleton, params, if, binops, user calls, print_*/str_eq/eq_fold intrinsics, let/var annotated+**inferred** via the Checker's `c.et`→`emit_ty_c`, `as` casts, distinct typedefs, **structs**: fwd typedef + `emit_struct_defs` + StructLit `.j_field` + Field access, push-based `emit_c_ty`). Growing allowlist, `DUMP_DIVERGE=1` to converge. plus increment 6: Float/Char/Bool/Null/Unary, str family (`.len`/`.ptr`, `s[lo..hi]`→substr, starts_with/…/trim intrinsics), `size_of`→`sizeof`. **NEXT:** Assign/Deref/slice-index/ArrayLit/FString/Try → loops → error sets → enums/match → traits/dyn → generics/mono → drop glue → closures → concurrency — the full worklist is in the P5 handoff "NEXT increments — everything still remaining" |
| R2 fixpoint | `--features selfhost-fixpoint` | 🟡 **HARNESS STANDS** — `selfhost_fixpoint_subset` (feature added, implies c-oracle): jc1 (Jestyr-written cgen) compiles every allowlisted program → C → gcc → run, asserts stdout+exit ≡ the Rust-compiled program. 9 runnable programs green. Full jc2≡jc3 lands when cgen.jtr covers the compiler sources |
