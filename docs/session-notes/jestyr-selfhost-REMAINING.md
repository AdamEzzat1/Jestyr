...
| P5 cgen | `src/cgen.rs` (~10.8K) | 🟡 **UNDERWAY (6 files)** — `examples/std/cgen.jtr` + golden `jestyr_cgen_matches_reference`; **byte-identical** on `{hello, bench_fib, eq_fold, distinct, compute, copy_optin}` (prelude+section skeleton, params, if, binops, user calls, print_*/str_eq/eq_fold intrinsics, let/var annotated+**inferred** via the Checker's `c.et`→`emit_ty_c`, `as` casts, distinct typedefs, **structs**: fwd typedef + `emit_struct_defs` + StructLit `.j_field` + Field access, push-based `emit_c_ty`). Growing allowlist, `DUMP_DIVERGE=1` to converge. **NEXT:** field-typed lets / `str`+slice in `emit_c_ty`, then `union`+`size_of`, enums/match, closures, concurrency — see the P5 handoff Progress log |
