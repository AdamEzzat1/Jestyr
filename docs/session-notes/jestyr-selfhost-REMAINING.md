...
| P5 cgen | `src/cgen.rs` (~10.8K) | 🟡 **UNDERWAY (4 files)** — `examples/std/cgen.jtr` + golden `jestyr_cgen_matches_reference`; **byte-identical** on `{hello, bench_fib, eq_fold, distinct}` (prelude+section skeleton, params, if, binops, user calls, print_*/str_eq/eq_fold intrinsics, let/var (annotated), `as` casts, distinct typedefs, push-based `emit_c_ty`). Growing allowlist, `DUMP_DIVERGE=1` to converge. **NEXT: structs** (`compute.jtr`) — plan in the P5 handoff Progress log (struct def/typedef, StructLit `.j_field`, Field access, inferred-let types via threading the Checker into the stmt group) |
