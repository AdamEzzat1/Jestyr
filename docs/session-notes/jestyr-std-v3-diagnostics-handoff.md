...
| **FIXED — see `jestyr-std-v4-runtime-platform-handoff.md` §2.2. The mechanism below is BACKWARDS: the field ACCESS was the site that survived; the field DECLARATION and every struct literal were what got renamed. One token-level exclusion in `ml_rewrite` (`ident :` is a binder, except `const NAME: T`) closed it — 43 → 49 of the multi-module corpus programs build, and `docs/jc_build_matrix.txt` now gates the set in both directions. The original text is kept below for its shape.** ~~`jc` cannot BUILD a program in which a top-level fn name is ALSO a struct field name.~~ Two modules defining a fn with the same name make the loader rename it; if that name is also a **struct field** accessed as `x.name` inside one of them, the rename rewrites the FIELD ACCESS too and it stops resolving, so the self-hosted typeck marks it Error and the driver refuses. `jestyrc` compiles and runs the same program | **Reference-vs-port divergence, `jc build` only. Measured 2026-08-20 at `cdf547a`: 9 of the 52 runnable multi-module corpus programs fail** — 5 this way, 4 later at gcc with undefined types (`Jestyr_Option__i32`, `JestyrFn_fn_di64_di64_ret_i64`; mechanism not isolated). Four-control repro: alongside `import "list"`, a plain `fn cap()` (a `List` FIELD only) is fine, `fn make()` (a `list` GENERIC only) is fine, an unrelated name is fine, and `fn len()` (**both**) breaks `l.len`/`l.cap` inside `list.jtr`. Three corpus instances: `len` (list+json+7), `refused` (fs+process+walk, breaks `fs.jtr:93`), `lost` (sink+test, breaks `sink.jtr:47`). **An earlier version of this row said "any program importing `std/json`" — that was too narrow; `caps_demo` imports no json and fails identically.** Same mechanism the repo already records for the flatten ("cannot tell `mod.item` from a field access on a local of the same name"), in a new position. Invisible to `CGEN_GOLDEN_ALLOWLIST`, which resolves no imports, AND to `selfhost_fixpoint_subset`, which `continue`s on any file containing `import "` — so these files are byte-identity verified *and* unbuildable. Full write-up: `docs/session-notes/census-demo-value-audit-2026-08-20.md` |
