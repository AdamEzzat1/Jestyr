...
1. **The lexer dogfood — the first real USE of tier 7 inside the compiler.** Replace the
   self-hosted lexer's `is_alpha`/`is_digit`/`is_hex`/`is_digit_us` branch chains
   ([tokens.jtr:47](examples/std/tokens.jtr:47)) with a comptime-computed 256-entry
   character-class table. This was blocked on M7 and is now unblocked: a class table
   needs its *shape* computed, and tier 6 alone would have meant 256 spelled-out
   literals — strictly worse than the branches it replaces. (The escape routes were
   checked and all close: `[0; 256]` cannot be filled without mutation, `[[0; 16]; 16]`
   gives identical entries, and neither implementation has list concatenation.)
   **This one edits a module INSIDE the self-host closure**, so it carries the fixpoint,
   the self-build and a seed refresh with it, and it is the increment that will actually
   say whether the tier was worth building — a computed table in the compiler's own
   lexer rather than a demo file. Keep it separate from anything else so a failure
   points at it.
2. **Widen `par for` past `i64` — the SIMD prerequisite.** `par for` reduces over
   `[]i64` only ([typeck.rs:2420](src/typeck.rs:2420)), because `spawn` targets cannot be
   generic — a constraint that lives in workstream N, not Q. On AVX2's 256-bit registers
   that caps a vector lowering at **4 lanes**; the same loops over `i32` would give 8 and
   over `u8` 32. So building Q-S2 first would ship the smallest version of the win, and
   the thing gating the bigger version is not SIMD work at all. Do this before the
   lowering.
