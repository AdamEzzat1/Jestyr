...
1. ~~**The G6 port mirror — the representation change.**~~ — **DONE (M6).** The
   ...
   **A finding that reorders what comes next.** The obvious dogfood for tier 6 — replacing
   the self-hosted lexer's `is_alpha`/`is_digit`/`is_hex`/`is_digit_us` branch chains
   ([tokens.jtr:47](examples/std/tokens.jtr:47)) with a 256-entry character-class table —
   **is blocked on M7, not M8.** A class table needs its *shape* computed, and tier 6
   computes only *values*: without a comptime `for` the alternative is spelling out 256
   literal entries, which is strictly worse than the branch chains it would replace. The
   escape routes were checked and all close: `[0; 256]` cannot be filled without
   mutation, nested repeats (`[[0; 16]; 16]`) give all-identical entries, and neither
   implementation has list concatenation. **So M7 comes before the dogfood, and the
   dogfood is what will prove the tier is worth having** — a computed table in the
   compiler's own lexer rather than a demo file.
